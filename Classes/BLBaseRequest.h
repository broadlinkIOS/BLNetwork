...
NS_ENUM(NSInteger) {
    BLRequestValidationErrorInvalidStatusCode = -8,
    BLRequestValidationErrorInvalidJSONFormat = -9,
};

///  HTTP Request method.
typedef NS_ENUM(NSInteger, BLRequestMethod) {
    BLRequestMethodGET = 0,
    BLRequestMethodPOST,
    BLRequestMethodHEAD,
    BLRequestMethodPUT,
    BLRequestMethodDELETE,
    BLRequestMethodPATCH,
};

///  Request serializer type.
typedef NS_ENUM(NSInteger, BLRequestSerializerType) {
    BLRequestSerializerTypeHTTP = 0,
    BLRequestSerializerTypeJSON,
};

///  Response serializer type, which determines response serialization process and
///  the type of `responseObject`.
typedef NS_ENUM(NSInteger, BLResponseSerializerType) {
    /// NSData type
    BLResponseSerializerTypeHTTP,
    /// JSON object type
    BLResponseSerializerTypeJSON,
    /// NSXMLParser type
    BLResponseSerializerTypeXMLParser,
};

///  Request priority
typedef NS_ENUM(NSInteger, BLRequestPriority) {
    BLRequestPriorityLow = -4L,
    BLRequestPriorityDefault = 0,
    BLRequestPriorityHigh = 4,
};
...
typedef void(^BLRequestCompletionBlock)(__kindof BLBaseRequest *request);

///  The BLRequestDelegate protocol defines several optional methods you can use
///  to receive network-related messages. All the delegate methods will be called
///  on the main queue.
@protocol BLRequestDelegate <NSObject>

@optional
///  Tell the delegate that the request has finished successfully.
///  called on main thread
///  @param request The corresponding request.
- (void)requestFinished:(__kindof BLBaseRequest *)request;

///  Tell the delegate that the request has failed.
///
///  @param request The corresponding request.
- (void)requestFailed:(__kindof BLBaseRequest *)request;

@end

///  The BLRequestAccessory protocol defines several optional methods that can be
...
@protocol BLRequestAccessory <NSObject>
...
///  BLBaseRequest is the abstract class of network request. It provides many options
///  for constructing request. It's the base class of `BLRequest`.
@interface BLBaseRequest : NSObject
...
///  `BLResponseSerializerType`. Note this value can be nil if request failed.
///
...
///  If you use `BLResponseSerializerTypeJSON`, this is a convenience (and sematic) getter
...
///  Default is nil.
@property (nonatomic, weak, nullable) id<BLRequestDelegate> delegate;
...
@property (nonatomic, copy, nullable) BLRequestCompletionBlock successCompletionBlock;

///  The failure callback. Note if this value is not nil and `requestFailed` delegate method is
///  also implemented, both will be executed but delegate method is first called. This block
///  will be called on the main queue.
@property (nonatomic, copy, nullable) BLRequestCompletionBlock failureCompletionBlock;

///
///  will be called on the background queue.
@property (nonatomic, copy, nullable) BLRequestCompletionBlock preprocessorBlock;

///  This can be used to add several accossories object. Note if you use `addAccessory` to add acceesory
///  this array will be automatically created. Default is nil.
@property (nonatomic, strong, nullable) NSMutableArray<id<BLRequestAccessory>> *requestAccessories;
...
///  The priority of the request. Effective only on iOS 8+. Default is `BLRequestPriorityDefault`.
@property (nonatomic) BLRequestPriority requestPriority;

///  Set completion callbacks
- (void)setCompletionBlockWithSuccess:(nullable BLRequestCompletionBlock)success
                              failure:(nullable BLRequestCompletionBlock)failure;
...
///  HTTP request method.
@property (nonatomic, assign) BLRequestMethod requestMethod;

///  Request serializer type.
@property (nonatomic, assign) BLRequestSerializerType requestSerializerType;

///  Response serializer type. See also `responseObject`.
@property (nonatomic, assign) BLResponseSerializerType responseSerializerType;
...
@property (nonatomic, assign) id jsonValidator;

@property (nonatomic, copy) NSDictionary *cacheFileNameArguments;
...
- (void)startWithCompletionBlockWithSuccess:(nullable BLRequestCompletionBlock)success
                                    failure:(nullable BLRequestCompletionBlock)failure;
