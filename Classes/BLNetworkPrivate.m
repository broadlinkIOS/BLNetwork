...
#ifdef DEBUG
    if (![BLNetworkConfig sharedConfig].debugLogEnabled) {
        return;
    }
    va_list argptr;
...
+ (void)addDoNotBackupAttribute:(NSString *)path {
    ...
    if (error) {
        BLLog(@"error to set do not backup attribute, error = %@", error);
    }
}
...
@implementation BLBaseRequest (RequestAccessory)

- (void)toggleAccessoriesWillStartCallBack {
    for (id<BLRequestAccessory> accessory in self.requestAccessories) {
